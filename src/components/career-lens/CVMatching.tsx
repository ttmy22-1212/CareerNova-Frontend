    ...
    <div className="p-6 space-y-5">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div className="p-5 border-b border-slate-100">
          <h2 className="font-bold text-slate-900 mb-0.5">Configure Analysis</h2>
          <p className="text-xs text-slate-500">Upload your CV and provide a job description or benchmark role to start the comparison.</p>
        </div>
        <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
          {/* CV Upload */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Your CV / Profile</label>
            {cvUploaded ? (
              <div className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-900 truncate">Jane_Doe_CV_2026.pdf</p>
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center hover:border-blue-300 hover:bg-blue-50/30 transition-all cursor-pointer">
                ...
                <p className="text-sm font-medium text-slate-700 mb-0.5">Drop your CV here</p>
          ...
          {/* JD Input */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Job Description Source</label>
              ...
              {modeButtons.map((b) => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg text-xs font-semibold transition-all ${
                    mode === b.key ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
                  }`}
                >
            ...
            {mode === "upload" && (
              <div className="border-2 border-dashed border-slate-200 rounded-xl p-5 text-center hover:border-blue-300 hover:bg-blue-50/30 transition-all cursor-pointer">
                ...
                <p className="text-xs font-medium text-slate-600">Upload JD document</p>
      ...
      {!analyzed ? (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-16 text-center">
          ...
          <p className="text-sm text-slate-500">Upload your CV and provide a job description to see your personalized match analysis.</p>
        </div>
      ) : (
        <>
          {/* ── Score Summary ── */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { label: "Overall Match", value: matchResult.overallScore, sub: "Strong candidate", type: "ring" },
              { label: "Skills Matched", value: `${matchResult.analysis.strongMatches.length}/${matchResult.analysis.strongMatches.length + matchResult.analysis.partialMatches.length}`, sub: "Core skills fully met", type: "text", color: "text-emerald-600" },
            ].map((card, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">{card.label}</p>
                {card.type === "ring" ? (
          ...
          <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-xl px-6 py-4 flex items-center justify-between">
            ...
            <Link href="/skill-gap" className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-left">
                      <p className="font-bold text-slate-900 text-sm">Strong Matches</p>
                      <p className="text-xs text-slate-500">Skills you've fully demonstrated</p>
                    </div>
                  ...
                  {expandedSection === "strong" ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                </button>
                {expandedSection !== "strong" && (
                  <div className="px-5 pb-4 flex flex-wrap gap-2">
                    {matchResult.analysis.strongMatches.map((item) => (
                      <span key={item.skill} className="px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-semibold rounded-full">{item.skill}</span>
                    ))}
                  </div>
                )}
                {expandedSection === "strong" && (
                  <div className="border-t border-slate-100 divide-y divide-slate-50">
                    {matchResult.analysis.strongMatches.map((item) => (
                      <div key={item.skill} className="px-5 py-3 flex items-center gap-4">
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-sm font-semibold text-slate-900">{item.skill}</span>
                            <span className="text-xs font-bold text-emerald-600">100% Match</span>
                          </div>
                          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full bg-emerald-500 rounded-full w-full" />
                          </div>
                          <div className="flex gap-4 mt-1.5 text-[11px] text-slate-500">
                            <span>Your level: <span className="font-semibold text-slate-700">{item.cvLevel}</span></span>
                            <span>Required: <span className="font-semibold text-slate-700">{item.required}</span></span>
              ...
              <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-left">
                      <p className="font-bold text-slate-900 text-sm">Partial Matches</p>
                      <p className="text-xs text-slate-500">Skills you have but need to level up</p>
                    </div>
                  ...
                  {expandedSection === "partial" ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                </button>
                {expandedSection !== "partial" && (
                  <div className="px-5 pb-4 flex flex-wrap gap-2">
                    {matchResult.analysis.partialMatches.map((item) => (
                      <span key={item.skill} className="px-3 py-1 bg-amber-100 text-amber-700 text-xs font-semibold rounded-full">{item.skill}</span>
                    ))}
                  </div>
                )}
                {expandedSection === "partial" && (
                  <div className="border-t border-slate-100 divide-y divide-slate-50">
                    {matchResult.analysis.partialMatches.map((item) => (
                      <div key={item.skill} className="px-5 py-3">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-sm font-semibold text-slate-900">{item.skill}</span>
                          <span className="text-xs font-bold text-amber-600">{item.match}% Match</span>
                        </div>
                        <div className="relative h-1.5 bg-slate-100 rounded-full overflow-hidden mb-1.5">
                          <div className="h-full bg-amber-400 rounded-full" style={{ width: `${item.match}%` }} />
                        </div>
                        ...
                        <div className="mt-2 flex items-start gap-1.5 p-2 bg-amber-50 rounded-lg">
                          ...
                          <p className="text-[11px] text-amber-700">Consider upskilling to reach the required level</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-left">
                      <p className="font-bold text-slate-900 text-sm">Missing Skills</p>
                      <p className="text-xs text-slate-500">Skills not yet in your profile</p>
                    </div>
                  ...
                  {expandedSection === "missing" ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                </button>
                {expandedSection !== "missing" && (
                  <div className="px-5 pb-4 flex flex-wrap gap-2">
                    {matchResult.analysis.missingSkills.map((item) => (
                      <span key={item.skill} className="px-3 py-1 bg-red-50 text-red-600 text-xs font-semibold rounded-full border border-red-200">{item.skill}</span>
                    ))}
                  </div>
                )}
                {expandedSection === "missing" && (
                  <div className="border-t border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-3 p-4">
                    {matchResult.analysis.missingSkills.map((item) => (
                      <div key={item.skill} className="p-3 bg-red-50 border border-red-100 rounded-xl">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-bold text-slate-900">{item.skill}</span>
                        </div>
                        <p className="text-[11px] text-slate-500 mb-1.5">Req: {item.required}</p>
                        <span className="px-2 py-0.5 bg-red-100 text-red-600 text-[10px] font-bold rounded-full">{item.priority}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
                ...
                <p className="text-xs text-slate-500 mb-4">You vs. Requirements</p>
                <ResponsiveContainer width="100%" height={220}>
                  <RadarChart data={matchResult.radarData} margin={{ top: 10, right: 20, bottom: 10, left: 20 }}>
                    <PolarGrid stroke="#e2e8f0" />
                    <PolarAngleAxis dataKey="subject" tick={{ fontSize: 10, fill: "#64748b" }} />
                    <PolarRadiusAxis tick={false} axisLine={false} domain={[0, 100]} />
                    <Radar name="You" dataKey="you" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.35} />
                    <Radar name="Required" dataKey="required" stroke="#10b981" fill="#10b981" fillOpacity={0.15} />
                    <Tooltip />
                  </RadarChart>
                </ResponsiveContainer>
                <div className="flex gap-4 justify-center mt-2">
                  <div className="flex items-center gap-1.5 text-xs text-slate-500">
                    <span className="w-4 h-0.5 bg-blue-500 rounded-full inline-block" />You
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-500">
                    <span className="w-4 h-0.5 bg-emerald-500 rounded-full inline-block" />Required
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-violet-50 to-indigo-50 border border-violet-100 rounded-xl p-5">
                <p className="text-sm font-bold text-violet-900 mb-1">Want detailed recommendations?</p>
                <p className="text-xs text-violet-700 mb-3">See exactly what to learn to close your skill gaps.</p>
                <Link href="/skill-gap" className="flex items-center gap-1.5 px-4 py-2 bg-violet-600 text-white rounded-lg text-xs font-semibold hover:bg-violet-700 transition-colors justify-center">
