...
const interests: { value: CareerInterest; label: string; Icon: any; tone: string }[] = [
...
const goals: { value: Goal; label: string; desc: string; Icon: any }[] = [
  { value: "internship", label: "Tìm thực tập", desc: "Internship trong 3-6 tháng tới", Icon: GraduationCap },
  { value: "fulltime", label: "Tìm việc fulltime", desc: "Bắt đầu sự nghiệp sau tốt nghiệp", Icon: Briefcase },
  { value: "switch", label: "Chuyển hướng", desc: "Đổi mảng kỹ thuật", Icon: Compass },
  { value: "explore", label: "Khám phá", desc: "Chưa rõ định hướng, muốn tìm hiểu", Icon: Sparkles },
];

const popularSkills = [
  "JavaScript", "TypeScript", "Python", "Java", "C++", "C#", "Go", "Rust",
  "React", "Next.js", "Vue", "Angular", "Node.js", "Express", "Spring Boot",
  "Django", "FastAPI", "Flask", "Tailwind CSS", "HTML/CSS",
...
const quizQuestions: {
...
}[] = [
  {
    ...
    options: [
      { text: "Giao diện người dùng đẹp, tương tác mượt", tags: ["frontend", "mobile"] },
      { text: "Logic phức tạp, hệ thống chịu tải lớn", tags: ["backend", "devops"] },
  ...
  },
  {
    ...
    options: [
      { text: "Visual & sáng tạo — thích thấy kết quả ngay", tags: ["frontend", "mobile"] },
      { text: "Logic & cấu trúc — thích tối ưu, refactor", tags: ["backend", "fullstack"] },
      { text: "Tỉ mỉ & phân tích — thích đào sâu data", tags: ["data", "ai_ml"] },
      { text: "Hệ thống & tự động — thích hạ tầng", tags: ["devops", "cybersecurity"] },
    ],
  },
  {
    ...
    options: [
      ...
      { text: "Phân tích pattern, viết test reproduce", tags: ["qa", "backend"] },
  ...
  },
  {
    ...
    options: [
      { text: "Senior Engineer, làm sản phẩm cho hàng triệu user", tags: ["fullstack", "frontend", "backend"] },
  ...
  },
  {
    ...
    options: [
      ...
      { text: "API service, microservices, tool nội bộ", tags: ["backend", "devops"] },
    ],
  },
];
...
export default function OnboardingWizard() {
  ...
  const canNext = useMemo(() => {
    ...
    return false;
  }, [step, profile, quizAnswers]);

  const addSkill = (name: string) => {
    if (!name.trim()) return;
    if (profile.topSkills.find((s) => s.name.toLowerCase() === name.toLowerCase())) return;
  ...
  const updateLevel = (name: string, level: number) => {
    setProfile({
      topSkills: profile.topSkills.map((s) => (s.name === name ? { ...s, level } : s)),
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 px-4 py-10">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Chào mừng đến Career Insight</h1>
        ...
        <div className="mb-6">
          <div className="mb-2 flex items-center justify-between text-xs font-medium">
            <span className="text-slate-600">Bước {step}/{TOTAL_STEPS}</span>
          ...
          <div className="mt-3 flex justify-between text-[10px] font-medium text-slate-500">
            {["Ngành học", "Định hướng", "Skill & CV", "Mục tiêu", "Quiz"].map((label, i) => (
              <span
                key={label}
                className={
                  i + 1 <= step ? "text-blue-600 font-semibold" : ""
                }
              >
                {label}
              </span>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          ...
          {step === 3 && (
            <Step3
              ...
              onCV={(name) =>
                setProfile({ hasUploadedCV: !!name, cvFileName: name })
              }
            />
          )}
          {step === 4 && (
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          {step < TOTAL_STEPS ? (
            ...
            >
              Hoàn tất <Check className="h-4 w-4" />
            </button>
          )}
        </div>
      </div>
...
function Step1({
  ...
  return (
    <div>
      <h2 className="mb-1 text-xl font-bold text-slate-900">Bạn học ngành gì?</h2>
      <p className="mb-5 text-sm text-slate-500">
        Chọn ngành gần nhất với chuyên môn của bạn để chúng tôi gợi ý career path phù hợp.
      </p>
...
function Step2({
  ...
  return (
    <div>
      <h2 className="mb-1 text-xl font-bold text-slate-900">Năm học & định hướng</h2>
      <p className="mb-5 text-sm text-slate-500">
      ...
      <div className="grid grid-cols-3 gap-2.5 md:grid-cols-4">
        {interests.map((it) => {
          ...
          return (
            ...
            >
              <it.Icon className={`h-5 w-5 ${active ? "text-blue-600" : "text-slate-500"}`} />
...
function Step3({
  ...
  onCV: (name: string | null) => void;
}) {
  const suggestions = popularSkills
    .filter((p) => !skills.find((s) => s.name === p))
    .filter((p) => !skillInput || p.toLowerCase().includes(skillInput.toLowerCase()))
    .slice(0, 6);

  return (
    <div>
      <h2 className="mb-1 text-xl font-bold text-slate-900">Skill & CV của bạn</h2>
      <p className="mb-5 text-sm text-slate-500">
        Khai báo 3-8 skill mạnh nhất + level. Upload CV để chúng tôi phân tích chính xác hơn (tùy chọn).
      </p>
        ...
        {hasCV ? (
          ...
          <label className="flex cursor-pointer flex-col items-center gap-1.5">
            ...
            <input
              ...
              accept=".pdf,.doc,.docx"
              className="hidden"
              onChange={(e) => onCV(e.target.files?.[0]?.name ?? null)}
            />
          </label>
        )}
      </div>
        ...
        <div className="mb-4 flex flex-wrap gap-1.5">
          {suggestions.map((s) => (
            <button
              key={s}
              onClick={() => addSkill(s)}
              className="rounded-md border border-slate-200 bg-white px-2 py-0.5 text-xs text-slate-600 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700"
            >
              + {s}
            </button>
          ))}
      ...
      <div className="space-y-2">
        ...
        {skills.length === 0 && (
          <p className="rounded-lg bg-slate-50 px-3 py-3 text-center text-xs text-slate-500">
            Chưa có skill nào. Gõ tên skill và bấm Enter, hoặc bấm vào gợi ý ở trên.
          </p>
        )}
      </div>
    </div>
...
function Step4({
  ...
  return (
    <div>
      <h2 className="mb-1 text-xl font-bold text-slate-900">Mục tiêu nghề nghiệp</h2>
      <p className="mb-5 text-sm text-slate-500">
          ...
          >
            <div
              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${
                goal === g.value ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"
              }`}
            >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <input
            type="number"
            value={targetSalary ?? ""}
            onChange={(e) => onSalary(e.target.value ? Number(e.target.value) : null)}
            placeholder="VD: 30000"
            className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
          />
          <p className="mt-1 text-[11px] text-slate-400">
            Ước tính ~{targetSalary ? Math.round((targetSalary * 25000) / 12 / 1000000) : "??"}{" "}
        ...
        <div className="flex items-center">
          <label className="flex cursor-pointer items-center gap-2.5 rounded-lg border border-slate-200 bg-white px-4 py-2.5">
            ...
            />
            <span className="text-sm font-medium text-slate-700">Ưu tiên job Remote</span>
          </label>
        </div>
      </div>
    </div>
...
function Step5({
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-slate-500">
        5 câu hỏi nhanh để tìm 2-3 hướng phù hợp nhất. ({completed}/{quizQuestions.length})
      </p>

      <div className="space-y-5">
        {quizQuestions.map((q, qIdx) => (
          <div key={qIdx} className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
